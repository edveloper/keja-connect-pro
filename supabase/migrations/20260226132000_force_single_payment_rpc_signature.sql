-- Force a single callable RPC signature for payment recording.
-- Some environments still had extra overloads with different argument order/types.
...
BEGIN
  FOR fn IN
    ...
    WHERE n.nspname = 'public'
      AND p.proname = 'record_payment_with_smart_allocation'
      AND pg_get_function_identity_arguments(p.oid) <> 'p_tenant_id uuid, p_amount integer, p_payment_month text, p_mpesa_code text, p_note text, p_user_id uuid, p_payment_date timestamp with time zone'
  LOOP
    EXECUTE format(
